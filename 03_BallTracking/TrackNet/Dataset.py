                    ...
                    for frame_number, frame in clip['frames_with_objects'].items():    
                        ...
                        for ball in frame['balls']:
                            if ball['visibility'] not in ['Outside'] and ball['trajectory'] not in ['', 'Static']:
                                points.append(scale_points([ball['x'],ball['y']], resolution[0] / resolution_subset[0]))
                        rows.append({
                            'subset': subset['name'],
                            'video': video['name'],
                            'clip': clip['name'],
...
class BallDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        # Example entry from JSON: {"video": "video1", "clip": "clip1", "frames": ["0.jpg", "1.jpg", "2.jpg"], "label": {"x": 100, "y": 150, "status": 1, "vis": 1}}
