...
def generate_heatmaps(image_size, keypoints, radius=3):
    h, w = image_size  # Image height and width
    heatmaps = np.zeros((len(keypoints), h, w), dtype=np.float32)

    for i, (px, py) in enumerate(keypoints):
        # Ensure keypoints are within image bounds
        if 0 <= px < w and 0 <= py < h:
            # Draw a circle of ones at the keypoint location
            heatmap = np.zeros((h, w), dtype=np.float32)
            center = (int(px), int(py))  # Keypoint location as (x, y)
            cv2.circle(heatmap, center, radius, 1.0, thickness=-1)  # Draw filled circle
            heatmaps[i] = heatmap
        else:
            heatmaps[i] = np.zeros((h, w), dtype=np.float32)
...
class CourtDatasetHeatmap(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Generate heatmaps
        heatmaps = torch.from_numpy(generate_heatmaps((self.model_height, self.model_width), points_tensor)).float()

        # Convert idx to tensor
        idx = torch.tensor(idx)

        return img, heatmaps, points_tensor, idx
...
def main():
    ...
    img,heatmaps, points, idx = dataset[sample_idx]
    ...
    # Plot the image and the first heatmap
    fig, axs = plt.subplots(1, 2, figsize=(12, 6))

    # Plot original image with keypoints
    axs[0].imshow(img_np)
    for point in points:
        axs[0].scatter(point[0], point[1], c="red", s=20)
    axs[0].set_title(f"Original Image - Sample {idx}")
    axs[0].axis("off")

    # Plot the first heatmap
    axs[1].imshow(heatmaps[0], cmap="hot")
    axs[1].set_title(f"First Heatmap")
    axs[1].axis("off")
